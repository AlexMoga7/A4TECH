...
stock Engine(playerid)
{
	...
	if(engine == 1)
	{
		...
		{
			format(string, sizeof(string), "* An unknown hitman stops the engine of his %s.", VehicleNames[vehicles]);
			nearByMessage(playerid, COLOR_PURPLE, string);
		}
	}
	else
	{
        ...
        if(Undercover[playerid] == 0)
        {
        	format(string, sizeof(string), "* %s starts the engine of his %s.", N(playerid), VehicleNames[vehicles]);
			nearByMessage(playerid, COLOR_PURPLE, string);
        }
        else
        {
			format(string, sizeof(string), "* An unknown hitman starts the engine of his %s.", VehicleNames[vehicles]);
			nearByMessage(playerid, COLOR_PURPLE, string);
		}
	}
	return 1;
